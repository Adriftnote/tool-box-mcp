...
function success(transformed) {
    return { transformed, needsAI: false };
}
function needsAI(data, hint) {
 ...
 * 새 변환 등록
 */
export function registerTransform(def) {
...
registerTransform({
    ...
    fn: (data) => {
        ...
        return success(arr.map((item) => item[firstKey]));
    }
});
...
registerTransform({
    ...
    fn: (data) => {
        ...
        if (!Array.isArray(arr))
            return success(data);
        const csv = arr.map((row) => row.map(cell => {
 ...
 */
export function applyTransform(data, transformType) {
    ...
    try {
        ...
        if (params.length > 0 && baseName === 'pick') {
            ...
            if (!Array.isArray(arr))
                return success(data);
            return success(arr.map((item) => {
                const picked = {};
                fields.forEach(f => { if (f in item)
                    picked[f] = item[f]; });
                return picked;
            }));
        }
