...
export class ChainBuilder {
  ...
  private options: ChainBuilderOptions;
  ...
  loadGraph(path?: string): void {
    ...
    try {
      ...
      throw new Error(`Failed to load knowledge graph: ${(error as Error).message}`);
    }
  }
  ...
  buildChain(tools: KnowledgeSource[]): BuiltChain {
    ...
    const toolHints: Record<string, Record<string, string>> = {};
   ...
   */
  private extractRelations(toolNames: string[]): Relation[] {
   ...
   */
  private topologicalSort(toolNames: string[], relations: Relation[]): string[] {
   ...
   */
  private extractDataFlow(relations: Relation[]): Array<{ from: string; to: string }> {
    return relations
   ...
   * Find matching usage pattern
   */
  private findMatchingPattern(toolNames: string[]): UsagePattern | undefined {
    if (!this.graph?.usage_patterns) return undefined;
  ...
  private buildSteps(
    ...
    const outputsTo = new Map<string, string[]>();
    for (const rel of relations) {
   ...
   */
  private buildToolArgs(toolName: string, isFirst: boolean): string {
  ...
  getRelations(toolName: string): Relation[] {
    ...
    return this.graph.relations.filter(
      rel => rel.from === toolName || rel.to === toolName
    );
  }
  ...
  suggestChain(taskDescription: string): BuiltChain | null {
    if (!this.graph?.usage_patterns) return null;
    ...
    for (const pattern of this.graph.usage_patterns) {
      ...
      if (matches.length >= 2) {
        ...
        const tools: KnowledgeSource[] = pattern.required_tools.map(name => {
          const entity = this.getEntity(name);
          return {
            name,
            type: (entity?.entityType as any) || 'Tool',
