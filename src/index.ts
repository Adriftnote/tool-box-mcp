...
import {
  ...
  ExpandGraphInputSchema,
  GetToolClusterInputSchema,
  RegisterToolInputSchema,
  DeleteToolInputSchema,
  ListToolsInputSchema,
  ExecuteChainInputSchema,
  PrepareChainInputSchema,
  ToolhubChainInputSchema,
  EmptyInputSchema,
  ResponseFormat,
  type SearchToolsInput,
  type ExpandGraphInput,
  type GetToolClusterInput,
...
async function ensureDiscoveryInitialized(): Promise<void> {
...
Use this tool when you need to find which tools are relevant for a task.`,
  ...
  async (params: SearchToolsInput) => {
    try {
      const result = await hybridSearch.search(
        params.query,
        params.limit,
        params.include_graph
      );
...
Use this tool when you know a primary tool and need to find related tools.`,
  ...
  async (params: ExpandGraphInput) => {
    try {
      const dependencies = await hybridSearch.expandTool(
        params.tool_name,
        params.depth,
...
Use this tool for complete task setup - returns everything needed to start working.`,
  ...
  async (params: GetToolClusterInput) => {
    try {
      const cluster = await hybridSearch.getToolCluster(
        params.query,
...
function formatSearchResultsMarkdown(result: { results: Array<{ name: string; type: string; description: string; similarity?: number }>; stats: { vectorCount: number; graphCount: number; totalCount: number; tokenEstimate: number; savingsPercent: number } }): string {
  const lines: string[] = [
    `# Search Results`,
    '',
...
function formatExpandResultMarkdown(toolName: string, dependencies: Array<{ name: string; type: string; description: string; metadata?: Record<string, unknown> }>): string {
  const lines: string[] = [
...
function formatClusterMarkdown(cluster: { primary: Array<{ name: string; type: string; description: string }>; dependencies: Array<{ name: string; type: string; description: string }>; context?: { usagePatterns: string[]; examples: string[] }; stats: { totalTools: number; tokenEstimate: number; savingsPercent: number } }): string {
  const lines: string[] = [
    `# Tool Cluster`,
    '',
...
Example for MCP_Server:
{
  ...
  async (params: RegisterToolInput) => {
    try {
  ...
  async (params: ListToolsInput) => {
    try {
      ...
      if (params.type_filter !== 'all') {
        filteredTools = filteredTools.filter((t: { type: string }) => t.type === params.type_filter);
      }
      ...
      for (const tool of paginatedTools) {
        const flags = [];
        if (tool.hasMcpCli) flags.push('mcp-cli');
        if (tool.hasSkillMeta) flags.push('skill');
...
Returns:
  ...
  async (params: ExecuteChainInput) => {
    try {
      let steps: ChainStep[];
      let builtChain: BuiltChain | null = null;
      ...
      for (const [toolName, hints] of Object.entries(toolHints) as [string, Record<string, string>][]) {
        if (hints.example) {
...
Returns the final result from the chain execution.`,
  ...
  async (params: ToolhubChainInput) => {
    try {
      await ensureDiscoveryInitialized();

      const steps: DiscoveryChainStep[] = params.mcpPath.map(s => ({
        toolName: s.toolName,
        toolArgs: s.toolArgs,
        inputPath: s.inputPath,
        outputPath: s.outputPath,
...
Use this to prepare context before toolhub_chain execution.`,
  ...
  async (params: PrepareChainInput) => {
    try {
      await ensureDiscoveryInitialized();
      ...
      const steps: Array<{
        ...
        relatedSkills?: Array<{ name: string; summary: string; relation: string }>;
      ...
      for (let i = 0; i < params.mcpPath.length; i++) {
        ...
        const stepInfo: typeof steps[0] = { toolName: step.toolName };

        // 1. Find tool schema
        if (params.include_schemas) {
          const tool = discoveredTools.find(t => t.name === step.toolName);
          if (tool) {
            stepInfo.inputSchema = tool.tool.inputSchema as Record<string, unknown>;
          } else {
        ...
        if (params.include_skills) {
          try {
            ...
            const expanded = await hybridSearch.expandTool(
              baseName,
              2,
              ['BENEFITS_FROM', 'WORKS_WITH']
            );
            ...
            if (skills.length > 0) {
              stepInfo.relatedSkills = skills.map(s => ({
                ...
                relation: (s.metadata?.relationType as string) || 'RELATED'
              }));
 ...
 */
function recommendTransform(sourceTool: string, targetTool: string): string | undefined {
...
Returns:
  ...
  async () => {
    const status: {
      chromadb: 'connected' | 'error' | 'unknown';
      chromadbError?: string;
      knowledgeGraph: 'loaded' | 'error' | 'not_found';
