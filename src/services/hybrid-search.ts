...
export class HybridSearchService {
  ...
  private graphSearch: GraphSearchService;
  ...
  async search(
    ...
    let graphResults: KnowledgeSource[] = [];
  ...
  async getToolCluster(
    ...
    const dependencies: KnowledgeSource[] = [];
   ...
   */
  private async findRelatedSkills(
    primary: KnowledgeSource[],
    dependencies: KnowledgeSource[]
  ): Promise<Array<{ name: string; summary: string; when: string; tokenSize: number }>> {
  ...
  private generateContext(
    primary: KnowledgeSource[],
    dependencies: KnowledgeSource[]
  ): { usagePatterns: string[]; examples: string[] } {
