...
export class ToolDiscoveryService {
     ...
     */
    async discoverServerTools(serverKey, serverConfig) {
        try {
     ...
     */
    async createToolClient(toolName) {
 ...
 */
function deepUnescape(str, depth = 0, maxDepth = 10) {
    try {
 ...
 */
export async function executeChain(steps, discoveryService) {
    let result = null;
    const trace = [];
            ...
            if (i === 0) {
                ...
                if (step.toolArgs.includes(`"${CHAIN_RESULT}"`)) {
                    ...
                    if (isValidJson && (Array.isArray(parsedResult) || typeof parsedResult === 'object')) {
                        toolInput = step.toolArgs.replace(`"${CHAIN_RESULT}"`, JSON.stringify(JSON.stringify(parsedResult)));
                    }
                    else {
