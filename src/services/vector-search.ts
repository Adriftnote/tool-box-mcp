...
export class VectorSearchService {
   ...
   */
  async search(query: string, limit: number = 3): Promise<KnowledgeSource[]> {
    try {
  ...
  private mapType(type: string): 'MCP_Server' | 'MCP_Server_Template' | 'Skill' | 'Tool' | 'Command' {
    const typeMap: Record<string, 'MCP_Server' | 'MCP_Server_Template' | 'Skill' | 'Tool' | 'Command'> = {
